...
/**
 * R1 password reset — set-new-password step. The recovery link lands here with
 * a PKCE `code`; we exchange it for a short-lived recovery session, let the
 * user set a new password, then sign out so they re-authenticate cleanly.
 */
export default function ResetUpdatePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (tokenHash) {
        // token_hash recovery (no PKCE verifier needed): verifyOtp establishes
        // a short-lived recovery session directly.
        ...
        // PKCE recovery: exchange the code (needs the verifier stored at request).
